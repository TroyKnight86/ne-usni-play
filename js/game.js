...
const HURRY = {
  ...
  corridor: 'Хватит бродить. Кофе — и за работу.',
  nursery: 'Тихо. Пусть спят. У тебя кофе и работа.',
};
...
SCENES.bathroom = { build() {
  ...
  glowHot({ left:'69.1%', top:'71.1%', width:'30.9%', height:'22.8%' },
    'polygon(69.1% 71.1%, 85.9% 71.3%, 100.0% 92.4%, 81.5% 93.9%)', 'ванна', () =>
    dialog('bath', [{ narr:true,
      t:'Ванна. Полежать бы. По статистике дома ты видишь её чаще всех — по семь минут в день.' }]));

  glowHot({ left:'73.9%', top:'3.9%', width:'11.6%', height:'67.2%' },
    'polygon(75.6% 4.3%, 85.5% 3.9%, 81.3% 70.4%, 73.9% 71.1%)', 'душ', () =>
  ...
  glowHot({ left:'0.7%', top:'38.4%', width:'14.5%', height:'60.8%' },
    'polygon(0.7% 38.8%, 14.2% 38.4%, 15.1% 99.1%, 1.0% 99.3%)', 'стиралка и цветы', () =>
...
SCENES.corridor = { build() {
  ...
  glowHot({ left:'11.8%', top:'46.1%', width:'15.2%', height:'10.0%' },
    ...
    dialog('desk', [
      ...
      { sp:'МАРК', t:'В своём доме кабинет будет с окном. В сад. Чтоб дышало.' },
  ...
  glowHot({ left:'7.0%', top:'24.3%', width:'8.6%', height:'19.8%' },
    'polygon(7.0% 24.3%, 15.4% 30.4%, 15.6% 44.1%, 7.1% 42.8%)', 'карта мира', () => gated('corridor', () =>
    dialog('worldmap', [
  ...
  // входная дверь теперь В КАДРЕ (левый край, корень v7) — не «за спиной»
  glowHot({ left:'0.0%', top:'14.9%', width:'2.9%', height:'80.0%' },
    'polygon(0.2% 14.9%, 2.9% 15.9%, 2.5% 92.1%, 0.0% 94.9%)', 'входная дверь',
    () => {
    if (S.time !== 'evening') {
  ...
  glowHot({ left:'89.8%', top:'17.8%', width:'6.5%', height:'78.5%' },
    'polygon(89.8% 21.6%, 95.7% 17.8%, 96.3% 96.3%, 90.0% 89.4%)', 'санузел', () => gated('corridor', () => {
    if (S.time !== 'evening') {
      dialog('wc', [
  ...
  // мини-хаб: детская — дверь в глубине (вход через хаб,
  // но СТЕНА детской — та, у которой стол: 40 см и гипсокартон живут)
  glowHot({ left:'57.4%', top:'30.9%', width:'4.8%', height:'43.0%' },
    'polygon(57.5% 30.9%, 62.2% 32.3%, 62.0% 71.3%, 57.4% 73.9%)', 'детская', () => goScene('nursery'));
  // переходы БЕЗ плашек (эксперимент 30.08: стираем границу игры) —
  // светящийся проём + подпись со стрелкой при наведении
  glowHot({ left:'63.8%', top:'29.9%', width:'7.7%', height:'41.7%' },
    'polygon(63.8% 29.9%, 71.4% 29.9%, 71.5% 71.6%, 63.9% 70.9%)', 'кухня ↑',
    () => goScene('kitchen'));
  glowHot({ left:'74.1%', top:'24.8%', width:'3.4%', height:'31.5%' },
    'polygon(74.1% 27.8%, 77.5% 24.8%, 77.5% 56.3%, 74.1% 52.1%)', 'гостиная →',
    () => goScene('living'));
  // спальня: плашка ЖДЁТ разметки Антона (за правым поворотом, канон — две двери)
...
SCENES.living = { build() {
  ...
  if (S.time === 'evening' && !S.flags.tvOn && !S.flags.boombox && !S.flags.litterSkipped) {
    ...
    hot({ left:'24%', top:'70%', width:'12%', height:'15%' },
      S.flags.timIntro ? 'Тим' : 'кот', () => {
      S.flags.catMet = true;
      let line;
      if (S.axis.ches >= 2) {
  ...
  glowHot({ left:'24.5%', top:'72.3%', width:'9.9%', height:'11.5%' },
    'polygon(24.5% 72.3%, 34.4% 72.3%, 34.4% 83.8%, 24.5% 83.8%)', 'лежанка Тимофея', () => gated('living', () => {
    if (!S.flags.timIntro) {
      S.flags.timIntro = true;
      dialog('cat_bed_intro', [
        { narr:true, t:'Лежанка Тимофея. Самого Тимофея на ней нет — он спит где угодно, кроме неё.' },
        { sp:'МАРК', t:'Тим — Верин трофей. Сам забрался в скорую, пока она была на вызове.\nУлица Тимофеева, между прочим. Считай, сам выбрал и семью, и имя.' },
        { narr:true, t:'Кот, который однажды решил, где будет жить, — и ни разу не усомнился.\nЕдинственный в этой квартире, у кого так получилось.' },
      ]);
    } else {
      dialog('cat_bed', [{ narr:true,
        t:'Лежанка Тимофея. Тима на ней нет — он спит где угодно, кроме неё.\nКупили лучшую. Он выбрал коробку. Знакомая жизненная стратегия, да?' }]);
    }
  }));
...
SCENES.nursery = { build() {
  ...
  glowHot({ left:'42%', top:'30%', width:'16%', height:'46%' },
    ...
    dialog('n_desk1', [
      { narr:true, t:'Максимов комп. RGB дышит даже во сне — тихая радуга под столом.\nКак аквариум. Только жрёт больше.' },
      { sp:'МАРК', t:'Макса бы на секцию записать. Который месяц собираюсь.' },
    ]));
  ...
  glowHot({ left:'74%', top:'56%', width:'25%', height:'36%' },
    'polygon(74% 56%, 99% 56%, 99% 92%, 74% 92%)', 'стол Алисы', () =>
    dialog('n_desk2', [
      { narr:true, t:'Алискин стол. Новый, с антресолью — подарок к школе.\nМонитор спит. Хозяйка тоже. Порядок в доме начинается с малого.' },
      { sp:'МАРК', t:'Интересно, как Алиска контрольную напишет. Готовилась вроде...\nну, как готовилась. По-алискиному.' },
    ]));
